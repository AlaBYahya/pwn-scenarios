#!/usr/bin/env python3
"""Collect writeup metadata from Medium publication RSS feeds.

Medium publications on a custom domain expose a standard RSS 2.0 feed at
`/feed` -- an explicitly publisher-provided syndication mechanism, not a
scrape. We only take title/link/author/date/categories from it, never the
full article body (each <item> also includes <content:encoded> with the
complete HTML text, which we deliberately never read past the metadata
fields -- same no-full-text policy as every other source).

Caveat: Medium's own feed only returns the ~10 most recent posts per
...
def main():
    ...
    ap.add_argument("--out", default="../data/raw/medium_feeds.jsonl")
    args = ap.parse_args()
